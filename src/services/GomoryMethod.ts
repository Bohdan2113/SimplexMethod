...
const MAX_GOMORY_ITERATIONS = 100;
const EPSILON = 1e-9;
...
function generateGomoryCut(
  ...
  const b_l = tableData.p0Values[rowIndex];
  ...
  const betaOverOneMinusBeta = beta_l / oneMinusBeta;
  ...
  for (let j = 0; j < a_lRow.length; j++) {
    ...
    if (varIndex < integerIndices.length && integerIndices.includes(varIndex)) {
      ...
      // Нецілочислова змінна (включаючи slack змінні)
      if (a_lj >= 0) {
        gamma_lj = a_lj;
      } else {
  ...
  const slackVariableName = `s${tableData.cValues.length + slackVarCount + 1}`;
